#!/usr/bin/env node
/**
 * M3b7 gate: every province/AR with ≥6 eligible non-capital prefecture_city
 * rows must have ≥6 live non-capital prefecture_city.
 ...
 * Xinjiang has 3 non-capital prefecture_city and is exempt to ≥6 (keep ≥3).
 * Ningxia has 4 and is exempt to ≥6 (keep ≥4).
 * Guizhou / Tibet have 5 and are exempt to ≥6 (keep ≥5).
 * Municipalities / SARs / Taiwan teaching packs are exempt.
 * This slice is M3b's sixth batch — not 293 complete, not 333 complete.
 */
...
function fail(msg) {
  console.error(`validate:m3b7 FAIL — ${msg}`)
  process.exitCode = 1
}
...
if (M3B7_NEW_LIVE.length < 18 || M3B7_NEW_LIVE.length > 28) {
  fail(`M3B7_NEW_LIVE ${M3B7_NEW_LIVE.length} not in 18–28`)
}
if (M3B7_NEW_LIVE.length > 30) fail(`M3B7_NEW_LIVE ${M3B7_NEW_LIVE.length} > 30 honest cap`)
if (new Set(M3B7_NEW_LIVE).size !== M3B7_NEW_LIVE.length) fail('duplicate M3B7 packId')
for (const id of M3B7_NEW_LIVE) {
  if (M3B6_NEW_LIVE.includes(id)) fail(`M3B7 pack ${id} already in M3B6_NEW_LIVE`)
  if (M3B5_NEW_LIVE.includes(id)) fail(`M3B7 pack ${id} already in M3B5_NEW_LIVE`)
  if (M3B4_NEW_LIVE.includes(id)) fail(`M3B7 pack ${id} already in M3B4_NEW_LIVE`)
  if (M3B3_NEW_LIVE.includes(id)) fail(`M3B7 pack ${id} already in M3B3_NEW_LIVE`)
  if (M3B2_NEW_LIVE.includes(id)) fail(`M3B7 pack ${id} already in M3B2_NEW_LIVE`)
  if (M3B1_CAPITAL_PACKS.includes(id)) fail(`M3B7 pack ${id} is an administrative center`)
}
...
if (!/≥6/.test(disc)) {
  fail('disclaimer must state 非省会 ≥6 / 省')
}
if (!/豁免青海|青海/.test(disc)) {
  fail('disclaimer must state 豁免青海')
}
if (!/海南|三沙/.test(disc)) {
  fail('disclaimer must state 豁免海南 / 三沙')
}
if (!/新疆/.test(disc)) {
  fail('disclaimer must state 豁免新疆')
}
if (!/宁夏/.test(disc)) {
  fail('disclaimer must state 豁免宁夏')
}
if (!/非省会总数 <6|非省会总数 <6 的省/.test(disc)) {
  fail('disclaimer must state 豁免非省会总数 <6 的省')
}
if (!/173\/293|约 173|prefecture_city live/.test(disc)) {
...
if (!/M3b7|第六批|批次管线/.test(disc + String(manifest.version || ''))) {
  fail('disclaimer/version must keep M3b7 batch pipeline wording')
}
...
for (const id of M3B4_NEW_LIVE) {
  if (!liveIds.has(id)) fail(`M3b4 new live missing ${id} (regression)`)
}
for (const id of M3B5_NEW_LIVE) {
  if (!liveIds.has(id)) fail(`M3b5 new live missing ${id} (regression)`)
}
for (const id of M3B6_NEW_LIVE) {
  if (!liveIds.has(id)) fail(`M3b6 new live missing ${id} (regression)`)
}
for (const id of M3B7_NEW_LIVE) {
  if (!liveIds.has(id)) fail(`M3b7 new live missing ${id}`)
}
...
if (gaps.length) fail(`provinces with ≥6 eligible non-capital prefecture_city but <6 live: ${gaps.join(', ')}`)
...
const xjCities = PREFECTURES.filter((r) => r.parentAdcode === '650000' && r.unitType === 'prefecture_city')
const xjEligible = eligibleNonCap(xjCities)
const xjLive = xjEligible.filter((r) => r.status === 'live' && r.playable_3d && r.packId)
if (xjEligible.length !== 3) fail(`新疆 eligible non-cap ${xjEligible.length} ≠ 3`)
if (xjLive.length < 3) fail('新疆 must keep ≥3 non-capital live (M3b4 no regression)')

const nxCities = PREFECTURES.filter((r) => r.parentAdcode === '640000' && r.unitType === 'prefecture_city')
const nxEligible = eligibleNonCap(nxCities)
const nxLive = nxEligible.filter((r) => r.status === 'live' && r.playable_3d && r.packId)
if (nxEligible.length !== 4) fail(`宁夏 eligible non-cap ${nxEligible.length} ≠ 4`)
if (nxLive.length < 4) fail('宁夏 must keep ≥4 non-capital live (M3b5 no regression)')

const gzCities = PREFECTURES.filter((r) => r.parentAdcode === '520000' && r.unitType === 'prefecture_city')
const gzEligible = eligibleNonCap(gzCities)
const gzLive = gzEligible.filter((r) => r.status === 'live' && r.playable_3d && r.packId)
if (gzEligible.length !== 5) fail(`贵州 eligible non-cap ${gzEligible.length} ≠ 5`)
if (gzLive.length < 5) fail('贵州 must keep ≥5 non-capital live (M3b6 no regression)')

const xzCities = PREFECTURES.filter((r) => r.parentAdcode === '540000' && r.unitType === 'prefecture_city')
const xzEligible = eligibleNonCap(xzCities)
const xzLive = xzEligible.filter((r) => r.status === 'live' && r.playable_3d && r.packId)
if (xzEligible.length !== 5) fail(`西藏 eligible non-cap ${xzEligible.length} ≠ 5`)
if (xzLive.length < 5) fail('西藏 must keep ≥5 non-capital live (M3b6 no regression)')
...
if (M3B7_NEW_LIVE.includes('sansha') || liveIds.has('sansha')) fail('三沙 must not be used to pad live counts')

if (M3B7_NEW_LIVE.includes('lijiang') || M3B7_NEW_LIVE.includes('zhangjiajie') || M3B7_NEW_LIVE.includes('dali')) {
  fail('celebrity mountain-city queue-jump in M3B7_NEW_LIVE')
}

if (process.exitCode) process.exit(process.exitCode)

console.log('validate:m3b7 OK — provinces with ≥6 eligible non-capital prefecture_city have ≥6 live, not 293/333 complete')
console.log(`  this-slice new live (${M3B7_NEW_LIVE.length}): ${M3B7_NEW_LIVE.join(', ')}`)
console.log(`  provinces covered ≥6: ${covered.join(', ')}`)
console.log(`  exempt (<6 eligible non-cap prefecture_city): ${exempt.join(', ')}`)
console.log(`  prefecture_city live ${pcLive.length}/293`)
console.log(`  M3b1 capitals still ${M3B1_CAPITAL_PACKS.length}; M3b2 still ${M3B2_NEW_LIVE.length}; M3b3 still ${M3B3_NEW_LIVE.length}; M3b4 still ${M3B4_NEW_LIVE.length}; M3b5 still ${M3B5_NEW_LIVE.length}; M3b6 still ${M3B6_NEW_LIVE.length}; M3a3 non-capital still ${M3A3_NONCAPITAL_LIVE.length}`)
