#!/usr/bin/env node
/**
 * M3b5 gate: every province/AR with ≥4 eligible non-capital prefecture_city
 * rows must have ≥4 live non-capital prefecture_city.
 ...
 * Xinjiang has 3 non-capital prefecture_city and is exempt to ≥4.
 * Municipalities / SARs / Taiwan teaching packs are exempt.
 * This slice is M3b's fourth batch — not 293 complete, not 333 complete.
 */
...
function fail(msg) {
  console.error(`validate:m3b5 FAIL — ${msg}`)
  process.exitCode = 1
}
...
if (M3B5_NEW_LIVE.length < 18 || M3B5_NEW_LIVE.length > 28) {
  fail(`M3B5_NEW_LIVE ${M3B5_NEW_LIVE.length} not in 18–28`)
}
if (M3B5_NEW_LIVE.length > 30) fail(`M3B5_NEW_LIVE ${M3B5_NEW_LIVE.length} > 30 honest cap`)
if (new Set(M3B5_NEW_LIVE).size !== M3B5_NEW_LIVE.length) fail('duplicate M3B5 packId')
for (const id of M3B5_NEW_LIVE) {
  if (M3B4_NEW_LIVE.includes(id)) fail(`M3B5 pack ${id} already in M3B4_NEW_LIVE`)
  if (M3B3_NEW_LIVE.includes(id)) fail(`M3B5 pack ${id} already in M3B3_NEW_LIVE`)
  if (M3B2_NEW_LIVE.includes(id)) fail(`M3B5 pack ${id} already in M3B2_NEW_LIVE`)
  if (M3B1_CAPITAL_PACKS.includes(id)) fail(`M3B5 pack ${id} is an administrative center`)
}
...
if (!/≥4/.test(disc)) {
  fail('disclaimer must state 非省会 ≥4 / 省')
}
...
if (!/非省会总数 <4|非省会总数 <4 的省/.test(disc)) {
  fail('disclaimer must state 豁免非省会总数 <4 的省')
}
if (!/129\/293|约 129|prefecture_city live/.test(disc)) {
...
if (!/M3b5|第四批|批次管线/.test(disc + String(manifest.version || ''))) {
  fail('disclaimer/version must keep M3b5 batch pipeline wording')
}
...
for (const id of M3B5_NEW_LIVE) {
  if (!liveIds.has(id)) fail(`M3b5 new live missing ${id}`)
}
...
if (gaps.length) fail(`provinces with ≥4 eligible non-capital prefecture_city but <4 live: ${gaps.join(', ')}`)
...
if (M3B5_NEW_LIVE.includes('sansha') || liveIds.has('sansha')) fail('三沙 must not be used to pad live counts')

if (M3B5_NEW_LIVE.includes('lijiang') || M3B5_NEW_LIVE.includes('zhangjiajie') || M3B5_NEW_LIVE.includes('dali')) {
  fail('celebrity mountain-city queue-jump in M3B5_NEW_LIVE')
}

if (process.exitCode) process.exit(process.exitCode)

console.log('validate:m3b5 OK — provinces with ≥4 eligible non-capital prefecture_city have ≥4 live, not 293/333 complete')
console.log(`  this-slice new live (${M3B5_NEW_LIVE.length}): ${M3B5_NEW_LIVE.join(', ')}`)
console.log(`  provinces covered ≥4: ${covered.join(', ')}`)
console.log(`  exempt (<4 eligible non-cap prefecture_city): ${exempt.join(', ')}`)
console.log(`  prefecture_city live ${pcLive.length}/293`)
console.log(`  M3b1 capitals still ${M3B1_CAPITAL_PACKS.length}; M3b2 still ${M3B2_NEW_LIVE.length}; M3b3 still ${M3B3_NEW_LIVE.length}; M3b4 still ${M3B4_NEW_LIVE.length}; M3a3 non-capital still ${M3A3_NONCAPITAL_LIVE.length}`)
