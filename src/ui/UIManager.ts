...
export class UIManager {
    ...
    this.layer.innerHTML = `
      <div class="overlay title-screen" data-ui="1">
        ...
        <div class="title-block">
          ...
          <p class="version">M3b12 每省非省会×11 · 收回散落的城市记忆</p>
        </div>
        ...
        <p class="footnote">不宣称全国地级已收录 · 34 省行政中心可玩 + 有地级市的省非省会 ≥11/省（豁免青海、海南、新疆、宁夏、贵州、西藏、吉林、云南、内蒙古、福建、陕西、河北、山西、浙江、江西、黑龙江与非省会总数 <11 的省） / 管线 / prefecture 约 257/293 · 不是 333 完成 · 不是 293 完成</p>
      </div>`
  ...
  private async renderLobby(): Promise<void> {
    ...
    const capitalLive = this.admin.provinces.filter((p) => this.provinceHasLiveCapital(p)).length
    const pc = this.prefectureCoverage()
    const chips = [
    ...
    if (open) {
      ...
      if (prov) {
        ...
        detail = `<div class="prov-detail">
          ...
          ${rest ? `<p class="hint">另有 ${rest} 座地级名录占位，记忆修复中。不是 333 完成，也不是 293 完成。</p>` : ''}
    ...
    this.layer.innerHTML = `
      <div class="overlay lobby-screen" data-ui="1">
        <div class="lobby-head">
          ...
          <div>
            <h2>全国大厅</h2>
            <p class="hint">34 省可点 · live 可进 3D · 草稿灰壳「记忆修复中」· 34 省行政中心可玩 + 有地级市的省非省会 ≥11/省（豁免青海、海南、新疆、宁夏、贵州、西藏、吉林、云南、内蒙古、福建、陕西、河北、山西、浙江、江西、黑龙江与非省会总数 <11 的省） / prefecture 约 ${pc.prefectureCityLive}/293 / 管线 · 不上未审中国全图</p>
          </div>
        </div>
        <div class="region-progress">
          <p>34 省行政中心可玩 ${capitalLive}/34 · 有地级市的省非省会 ≥11/省 ${pc.covered11}/${pc.need11}（豁免青海、海南、新疆、宁夏、贵州、西藏、吉林、云南、内蒙古、福建、陕西、河北、山西、浙江、江西、黑龙江与非省会总数 <11 的省） · prefecture_city live ${pc.prefectureCityLive}/293 · 七大区可玩 ${liveRegions}/7 · 非省会 live ${nonCapLive}（≥21） · 管线 ${pipeline} 座 · <b>不是 333 完成 · 不是 293 完成</b></p>
  ...
  private isNonCapitalCity(c: AdminPrefecture): boolean {
    ...
    const name = String(c.name || '').replace(/主城$/, '').replace(/教学包$/, '')
    const cap = prov.capital
    return !(name === cap || name === `${cap}市` || name.startsWith(cap))
  }

  private provinceHasLiveCapital(p: { adcode: string; capital: string }): boolean {
    const cap = p.capital
    return this.liveCitiesOf(p.adcode).some((c) => {
      const name = String(c.name || '').replace(/主城$/, '').replace(/教学包$/, '')
      return name === cap || name === `${cap}市` || name.startsWith(cap)
    })
  }
  ...
  private prefectureCoverage(): {
    ...
    for (const p of this.admin.provinces) {
      ...
      // 三沙永不可玩，不计入「可玩非省会 ≥3 / ≥4 / ≥5 / ≥6 / ≥7 / ≥8 / ≥9 / ≥10 / ≥11」分母。
      const eligible = nonCap.filter((c) => c.adcode !== '460300' && c.name !== '三沙市')
      const liveEligible = eligible.filter((c) => c.playable_3d && c.packId)
      if (eligible.length >= 3) {
        need3 += 1
        if (liveEligible.length >= 3) covered3 += 1
      }
      if (eligible.length >= 4) {
        need4 += 1
        if (liveEligible.length >= 4) covered4 += 1
      }
      if (eligible.length >= 5) {
        need5 += 1
        if (liveEligible.length >= 5) covered5 += 1
      }
      if (eligible.length >= 6) {
        need6 += 1
        if (liveEligible.length >= 6) covered6 += 1
      }
      if (eligible.length >= 7) {
        need7 += 1
        if (liveEligible.length >= 7) covered7 += 1
      }
      if (eligible.length >= 8) {
        need8 += 1
        if (liveEligible.length >= 8) covered8 += 1
      }
      if (eligible.length >= 9) {
        need9 += 1
        if (liveEligible.length >= 9) covered9 += 1
      }
      if (eligible.length >= 10) {
        need10 += 1
        if (liveEligible.length >= 10) covered10 += 1
      }
      // 黑龙江非省会 11 座里只剩伊春，山城秀禁选，不进 ≥11 分母。
      if (eligible.length >= 11 && p.adcode !== '230000') {
        need11 += 1
        if (liveEligible.length >= 11) covered11 += 1
      }
    }
    const prefectureCityLive = this.admin.prefectures.filter(
      (c) => c.unitType === 'prefecture_city' && c.playable_3d && c.packId,
    ).length
    return { need, covered, need2, covered2, need3, covered3, need4, covered4, need5, covered5, need6, covered6, need7, covered7, need8, covered8, need9, covered9, need10, covered10, need11, covered11, prefectureCityLive }
  }
